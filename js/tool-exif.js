...
this.ifd0Offset: ${this.ifd0Offset}, file.byteLength: ${e2.byteLength}`), e2.tiff && await e2.ensureChunk(this.ifd0Offset, S(this.options));
  ...
  (() => {
    ...
    const checkAll = document.getElementById("tool-check-all");
    if (!dropzone || !input || !list || !exportBtn || !clearBtn || !checkAll) return;
    ...
    const createRow = (entry) => {
      ...
      <td class="tool-download-cell">-</td>
    `;
      list.appendChild(row);
      const checkbox = row.querySelector(".tool-row-check");
      checkbox.addEventListener("change", () => {
        syncCheckAll();
      });
    };
    const getRow = (entryId) => list.querySelector(`tr[data-id="${entryId}"]`);
    const updateOriginal = (entry, text) => {
      const row = getRow(entry.id);
      if (!row) return;
      const cell = row.querySelector(".tool-original");
      if (cell) cell.textContent = text;
    };
    const updateStatus = (entry, text) => {
      const row = getRow(entry.id);
      if (!row) return;
      const cell = row.querySelector(".tool-status");
      if (cell) cell.textContent = text;
    };
    const updateResult = (entry, resultText) => {
      const row = getRow(entry.id);
      if (!row) return;
      const cell = row.querySelector(".tool-result");
      if (cell) cell.textContent = resultText;
    };
    const updateDownload = (entry, outputUrl, outputWidth) => {
      const row = getRow(entry.id);
      if (!row) return;
      const cell = row.querySelector(".tool-download-cell");
      if (!cell) return;
      const ext = entry.outputType === "image/png" ? "png" : "jpg";
      const base = entry.file.name.replace(/\.[^/.]+$/, "");
      const filename = `${base}_w${outputWidth}${entry.exifEnabled ? "_exif" : ""}.${ext}`;
      cell.innerHTML = `<a class="tool-download" download>\uB2E4\uC6B4\uB85C\uB4DC</a>`;
      const link = cell.querySelector("a");
      link.href = outputUrl;
      link.download = filename;
    };
    const readRowSettings = (entry) => {
      const row = getRow(entry.id);
    ...
    const addFiles = (files) => {
      Array.from(files).forEach((file) => {
        ...
        const entry = { id, file, width: 0, height: 0, objectUrl: "", outputUrl: "" };
        state.set(id, entry);
        createRow(entry);
        const img = new Image();
        ...
        img.onload = () => {
          ...
          updateOriginal(entry, `\uC6D0\uBCF8 ${entry.width}x${entry.height} \xB7 ${formatBytes(file.size)}`);
          updateStatus(entry, "\uB300\uAE30");
        };
        img.onerror = () => {
          updateOriginal(entry, "\uC774\uBBF8\uC9C0 \uB85C\uB4DC \uC2E4\uD328");
          updateStatus(entry, "\uC9C0\uC6D0\uB418\uC9C0 \uC54A\uB294 \uD30C\uC77C");
        };
        img.src = url;
      });
    };
    const resizeEntry = async (entry) => {
      if (!entry.width || !entry.height) {
        updateStatus(entry, "\uC6D0\uBCF8 \uC815\uBCF4 \uC5C6\uC74C");
        return;
      }
      ...
      if (settings.exif) {
        try {
          const exifData = await full_esm_default.parse(entry.file);
          exifText = formatExif(exifData) || "No EXIF info";
        } catch (err) {
          exifText = "No EXIF info";
        }
      }
      let img;
      try {
        img = await loadImage(entry.objectUrl);
      } catch (err) {
        updateStatus(entry, "\uC9C0\uC6D0\uB418\uC9C0 \uC54A\uB294 \uC774\uBBF8\uC9C0");
        return;
      }
      ...
      if (settings.exif && exifText) {
        const padding = 12;
        const lineHeight = 16;
        ctx.font = '12px "Hack", "IBMPlexSansKR-Regular", monospace';
        lines = wrapText(ctx, exifText, targetWidth - padding * 2);
        barHeight = padding * 2 + lineHeight * lines.length;
      }
      ...
      if (settings.exif && exifText) {
        ...
        lines.forEach((line, index) => {
          ctx.fillText(line, padding, targetHeight + padding + index * 16);
        });
      }
      const outputType = entry.file.type === "image/png" ? "image/png" : "image/jpeg";
      entry.outputType = outputType;
      const blob = await new Promise((resolve) => {
        canvas.toBlob(resolve, outputType, 0.92);
      });
      if (!blob) {
        updateStatus(entry, "\uB9AC\uC0AC\uC774\uC9D5 \uC2E4\uD328");
        return;
      }
      if (entry.outputUrl) URL.revokeObjectURL(entry.outputUrl);
      entry.outputUrl = URL.createObjectURL(blob);
      updateResult(entry, `${canvas.width}x${canvas.height} \xB7 ${formatBytes(blob.size)}`);
      updateDownload(entry, entry.outputUrl, canvas.width);
    };
    const getCheckedEntries = () => {
      const rows = Array.from(list.querySelectorAll("tr"));
      const entries = [];
      rows.forEach((row) => {
        const id = row.dataset.id;
        const check = row.querySelector(".tool-row-check");
        if (check && check.checked && state.has(id)) {
    ...
    const exportChecked = async () => {
      const entries = getCheckedEntries();
      if (entries.length === 0) return;
      exportBtn.disabled = true;
      for (const entry of entries) {
        try {
          updateStatus(entry, "\uB9AC\uC0AC\uC774\uC9D5 \uC911...");
          await resizeEntry(entry);
          const row = getRow(entry.id);
          const status = row?.querySelector(".tool-status")?.textContent;
          if (status === "\uB9AC\uC0AC\uC774\uC9D5 \uC911...") updateStatus(entry, "\uC644\uB8CC");
        } catch (err) {
          updateStatus(entry, "\uB9AC\uC0AC\uC774\uC9D5 \uC2E4\uD328");
        }
      }
    ...
    const clearAll = () => {
      Array.from(state.keys()).forEach((id) => removeEntry(id));
    };
    const syncCheckAll = () => {
      const checks = Array.from(list.querySelectorAll(".tool-row-check"));
      if (checks.length === 0) {
        checkAll.checked = false;
        return;
      }
      checkAll.checked = checks.every((c2) => c2.checked);
    };
